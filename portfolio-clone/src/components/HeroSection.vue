...
<template>
  <section id="hero" class="container mx-auto max-w-3xl px-5 sm:px-6 pt-6 pb-4" aria-label="Introduction">
    <div class="animate-in-up-on-view flex flex-col gap-5">
      ...
      <!-- Spotify Vinyl Record Mini-Widget -->
      <div class="flex flex-wrap items-stretch justify-between gap-5 pt-2">
        <div class="group w-full max-w-xs">
          <div class="flex items-center gap-3 rounded-xl border border-border/70 bg-card/80 p-2.5 shadow-sm backdrop-blur-sm">
            <!-- Vinyl Album Cover & Spinning Disc -->
            <div class="relative flex h-14 w-16 shrink-0 items-center">
              <div class="relative z-10 h-14 w-14 overflow-hidden rounded-[4px] shadow-md ring-1 ring-black/10 bg-neutral-900">
                <img 
                  src="/co2bien.png" 
                  alt="Current Focus & Music Cover" 
                  class="size-full object-cover"
                />
                <div class="pointer-events-none absolute inset-y-0 right-0 w-2 bg-gradient-to-l from-black/30 to-transparent"></div>
              </div>

              <!-- Vinyl Disc sticking out and spinning on hover -->
              <div class="absolute -right-0.5 top-1/2 z-0 -translate-y-1/2">
                <div class="relative size-12 shrink-0">
                  <!-- Spinning vinyl texture -->
                  <div class="absolute inset-0 rounded-full vinyl-disc group-hover:animate-vinyl-spin"></div>
                  <!-- Inner vinyl label -->
                  <div class="absolute inset-[26%] rounded-full bg-neutral-800 ring-1 ring-black/60"></div>
                  <!-- Center spindle hole -->
                  <div class="absolute left-1/2 top-1/2 z-10 size-1.5 -translate-x-1/2 -translate-y-1/2 rounded-full bg-neutral-300 shadow-sm ring-1 ring-neutral-400/80"></div>
                </div>
              </div>
            </div>
            ...
            <div class="min-w-0 flex-1 pl-1">
              <p class="flex items-center gap-1 font-mono text-[10px] text-secondary">
                <span class="size-1.5 rounded-full bg-[#1DB954] animate-pulse"></span>
                {{ t('spotifyLastPlayed') }}
              </p>
              <a 
                :href="t('spotifyUrl')" 
                target="_blank" 
                rel="noopener noreferrer" 
                class="mt-0.5 block truncate text-xs font-semibold text-foreground hover:underline"
              >
                {{ t('spotifyTrack') }}
              </a>
            ...
            <a 
              ...
              class="shrink-0 text-[#1DB954] transition-opacity hover:opacity-80 p-1"
            >
